...
def print_terminal_report(report: dict, *, detail_level: int = 0) -> None:
    ...
    _print_doc_indicators(module_results, detail_level)
...
# ---------------------------------------------------------------------------
# Office Document Indicators (doc_analysis summary)
# ---------------------------------------------------------------------------


def _print_doc_indicators(module_results: list[dict], detail_level: int) -> None:
    """Surface the structural Office-document indicators — VBA macro
    summary (incl. stomping), XLM deobfuscation, template injection,
    embedded OLE objects, and oleid container flags.

    Hidden when no Office document was analysed.
    """
    doc = next(
    ...
    sev_styles = {"bad": "red", "warn": "yellow", "info": "dim"}

    has_serious = any(sev != "info" for _, _, sev in rows)
    if detail_level < 1 and has_serious:
        rows_to_show = [r for r in rows if r[2] != "info"]
        # Always keep format/classification for context.
        for r in rows:
